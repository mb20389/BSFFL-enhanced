...
export default function Season() {
  ...
  return (
    <main style={{ padding: 24, fontFamily: "Inter, system-ui, sans-serif" }}>
      <Nav />
      <h1 style={{ marginBottom: 12, fontSize: 24, fontWeight: 700 }}>Season Standings (All-Play)</h1>

      {!LEAGUE_ID && (
        <div style={{ marginBottom: 12, color: "#b91c1c" }}>
          Missing NEXT_PUBLIC_SLEEPER_LEAGUE_ID
        </div>
      )}
              ...
              {rows.map((r, idx) => (
                <tr key={r.roster_id} style={{ borderBottom: "1px solid #f0f0f0" }}>
                  ...
                  <td style={{ padding: 8, display: "flex", alignItems: "center", gap: 8 }}>
                    {r.avatar && <img src={r.avatar} alt={r.custom_team_name} style={{ width: 36, height: 36, borderRadius: "50%" }} />}
